...
class LaTeXBuilder(IRBuilder):
    ...
    def __init__(self, image_map: dict[str, Path] | None = None) -> None:
    ...
    def _inline_from_pandoc(self, il: dict) -> InlineUnion | list[InlineUnion] | None:
        ...
        if t == "Str":
            ...
            # Render as text with citation markers
            if inner:
                return inner
            return None
        elif t == "Note":
    ...
    def _resolve_image_src(self, src: str) -> str:
        ...
        if src in self.image_map:
            return str(self.image_map[src])
        # Try filename match
        if path_obj.name in self.image_map:
            return str(self.image_map[path_obj.name])
        # Try stem match
        if path_obj.stem in self.image_map:
            return str(self.image_map[path_obj.stem])
